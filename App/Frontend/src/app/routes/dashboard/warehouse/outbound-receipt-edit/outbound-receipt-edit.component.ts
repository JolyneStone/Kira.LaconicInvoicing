...
@Component({
  ...
  templateUrl: './outbound-receipt-edit.component.html',
  styles: []
})
export class OutboundReceiptEditComponent extends ComponentBase implements OnInit {
  constructor(
    ...
    super(injector);
    this.outboundReceiptDto = new OutboundReceiptDto();
    this.outboundReceiptDto.items = [];
  }

  mode: 'Add' | 'Update' = 'Add';
  outboundReceiptDto: OutboundReceiptDto;
  editIndex = -1;
  editObj: OutboundReceiptItemDto;
  warehouseId: string;
  private isAdding = false;

  protected AuthConfig(): AuthConfig {
    const authConfig = new AuthConfig('Root.Warehouse.Warehouse.OutboundReceipt', []);
    if (this.mode === 'Add') {
  ...
  ngOnInit() {
    this.route.params.subscribe((params) => {
      if (params && params['id']) {
        this.mode = 'Update';
        this.warehouseService.getOutboundReceipt(params['id'])
          .subscribe(res => {
            if (res && res.type === AjaxResultType.success) {
              this.outboundReceiptDto = res.data;
  ...
  getNewNumber() {
    this.warehouseService.getOutboundReceiptNewNumber()
      .subscribe(res => {
        if (res && res.type === AjaxResultType.success) {
          this.outboundReceiptDto.number = res.data;
        } else if (res && res.content) {
          this.msg.error(res.content);
        }
      });
      ...
      nzFooter: [
        ...
        },
        {
          ...
          onClick: componentInstance => {
            ...
            this.outboundReceiptDto.warehouseName = data.name;
            this.outboundReceiptDto.warehouseNumber = data.number;
            this.outboundReceiptDto.items = [];
    ...
    const modal = this.modalService.create({
      ...
      nzComponentParams: {
        ...
        read: (query: QueryCondition) => {
          ...
          } else if (!componentInstance.auth.Search) {
            this.msg.warning('您没有该权限');
          } else {
            this.warehouseService.searchInventoryProduct({ warehouseId: this.warehouseId, goodsCategory: GoodsCategory.material }, null, query.pageIndex, query.pageSize)
              .subscribe(res => {
                if (res.type === AjaxResultType.success) {
                  componentInstance.data = res.data.rows;
                  componentInstance.total = res.data.total;
      ...
      nzFooter: [
        ...
        },
        {
          ...
          onClick: componentInstance => {
            ...
            if (!this.editObj) {
              this.editObj = new OutboundReceiptItemDto();
            }
      ...
      nzComponentParams: {
        ...
        read: (query: QueryCondition) => {
          ...
          if (!this.warehouseId) {
            ...
            this.warehouseService.searchInventoryMaterial({ warehouseId: this.warehouseId, goodsCategory: GoodsCategory.material }, null, query.pageIndex, query.pageSize)
              .subscribe(res => {
                if (res.type === AjaxResultType.success) {
                  componentInstance.data = res.data.rows;
                  componentInstance.total = res.data.total;
  ...
  addItem() {
    ...
    this.isAdding = true;
    this.editItem(this.outboundReceiptDto.items.length - 1);
  }

  delItem(index: number) {
    if (this.outboundReceiptDto.items && this.outboundReceiptDto.items.length < index)
      this.outboundReceiptDto.items = this.outboundReceiptDto.items.splice(index, 1);
  }

  editItem(index: number) {
    this.editObj = { ...this.outboundReceiptDto.items[index] };
  ...
  cancelItem(index: number) {
    this.editObj = {};
    if (this.isAdding) {
      this.outboundReceiptDto.items = this.outboundReceiptDto.items.splice(0, index);
      this.isAdding = false;
    }
    this.editIndex = -1;
  }
  ...
  submit() {
    if (!this.outboundReceiptDto.items || this.outboundReceiptDto.items.length <= 0) {
      this.msg.warning('请添加出库项');
      return;
    }

    if (this.mode === 'Add') {
      this.warehouseService.addOutboundReceipt(this.outboundReceiptDto)
        .subscribe(res => {
          if (res && res.type === AjaxResultType.success) {
      ...
      this.warehouseService.updateOutboundReceipt(this.outboundReceiptDto)
        .subscribe(res => {
          if (res && res.type === AjaxResultType.success) {
  ...
  startPrint() {
    ...
    this.router.navigate(['dashboard', 'print', 'print-window'], {
      queryParams: {
        id: this.outboundReceiptDto.id,
        type: TemplateType.outboundReceipt
      }
    });
  }
