import { VendorEditComponent } from './../vendor-edit/vendor-edit.component';
import { PurchaseService } from './../../../../shared/business/services/purchase.service';
import { VendorDto, VendorQueryDto, SortDescription } from './../../../../shared/business/app.model';
import { BusinessService } from '@shared/business/services/business.service';
import { ComponentBase } from '@shared/osharp/services/osharp.service';
import { Component, OnInit, Injector, ChangeDetectorRef, Input, ViewChild } from '@angular/core';
import { _HttpClient, SettingsService } from '@delon/theme';
import { NzMessageService, NzCollapsePanelComponent } from 'ng-zorro-antd';
import { I18NService } from '@core/i18n/i18n.service';
import { AuthConfig, AjaxResult, AjaxResultType } from '@shared/osharp/osharp.model';
import { forEach } from '@angular/router/src/utils/collection';

@Component({
  selector: 'app-vendor-list',
  templateUrl: './vendor-list.component.html',
  styles: []
})
export class VendorListComponent extends ComponentBase implements OnInit {

  constructor(
    private msg: NzMessageService,
    private cdr: ChangeDetectorRef,
    private bsr: BusinessService,
    private purchaseService: PurchaseService,
    injector: Injector,
  ) {
    super(injector);
    this.queryData = { number: '', name: '', type: '' };
    this.vendorDtoList = [];
  }
  @ViewChild('vendorEdit') vendorEdit: VendorEditComponent;
  vendorDtoList: VendorDto[];
  queryData: VendorQueryDto;
  loading = false;
  expandForm = false;
  types: any[];
  selectedRows: any[] = [];
  mapOfCheckedId: Map<string, boolean> = new Map<string, boolean>(); //{ [key: string]: boolean } = {};
  hasSelected = false;
  isIndeterminate = false;
  isAllDisplayDataChecked = false;
  pageSize = 10;
  pageIndex = 1;
  total = 0;
  sorts: SortDescription[] = [
    { field: 'Name', order: null },
    { field: 'Number', order: null },
    { field: 'Type', order: null },
    { field: 'Datetime', order: null }
  ];

  protected AuthConfig(): AuthConfig {
    return new AuthConfig('Root.Purchase.Purchase.Vendor', [
      'Search',
      'Add',
      'Get',
      'Delete'
    ]);
  }

  async ngOnInit() {
    await super.checkAuth();
    this.bsr.getBaseDataValuesByType('VENDORTYPE').subscribe(res => {
      this.types = res;
      this.cdr.detectChanges();
    });

    this.search();
  }

  sort(sortName: string, order: string): void {
    if (this.sorts.some(s => s.field === sortName)) {
      this.sorts = this.sorts.map(s => {
        if (s.field === sortName) {
          s.order = order;
        }
        return s;
      });
    }

    this.search();
  }

  hasRowSelected(): boolean {
    for (const v of this.mapOfCheckedId.values()) {
      if (v === true)
        return true;
    }

    return false;
  }

  search() {
    if (this.auth.Search) {
      this.purchaseService.searchVendor(this.queryData, this.sorts, this.pageIndex, this.pageSize)
        .subscribe((res: AjaxResult) => {
          if (res.type === AjaxResultType.success) {
            this.vendorDtoList = res.data.rows;
            this.total = res.data.total;
          } else if (res.content) {
            this.msg.error(res.content);
          }
        });
    }
  }

  refresh() {
    this.pageIndex = 1;
    this.queryData = { number: '', name: '', type: '' };
    this.search();
  }

  reset() {
    this.queryData = { number: '', name: '', type: '' };
  }

  currentPageDataChange($event: VendorDto[]) {
    this.vendorDtoList = $event;
    this.refreshStatus();
  }

  pageIndexChange(event) {
    this.pageIndex = event;
    this.search();
  }

  pageSizeChange(event: number) {
    this.pageSize = event;
  }

  checkAll(value: boolean): void {
    this.vendorDtoList.forEach(item => (this.mapOfCheckedId[item.id] = value));
    this.refreshStatus();
  }

  refreshStatus(): void {
    this.hasSelected = this.hasRowSelected();
    this.isAllDisplayDataChecked = this.vendorDtoList.every(item => this.mapOfCheckedId[item.id]);
    this.isIndeterminate =
      this.vendorDtoList.some(item => this.mapOfCheckedId[item.id]) && !this.isAllDisplayDataChecked;
  }

  add() {
    this.vendorEdit.show('Add');
  }

  edit(data: VendorDto) {
    this.purchaseService.getVendor(data.id)
      .subscribe(res => {
        if (res.type === AjaxResultType.success) {
          this.vendorEdit.show('Update', res.data);
        } else if (res.content) {
          this.msg.error(res.content);
        }
      });
  }

  delete() {
    const ids = this.vendorDtoList.filter(v => this.mapOfCheckedId[v.id]).map(v => v.id);
    if (!ids || ids.length === 0) {
      this.msg.warning('请选择要操作的数据');
    } else {
      this.purchaseService.deleteVendor(ids)
        .subscribe((res: any) => {
          if (res.type === AjaxResultType.success) {
            this.msg.success('删除成功');
          } else if (res.content) {
            this.msg.error(res.content);
          }
        });
    }
  }
}
